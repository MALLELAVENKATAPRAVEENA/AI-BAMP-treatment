...
fun DashboardScreen(
    ...
    Column(
        ...
        LazyColumn(
            ...
            // Real-Time Firestore Test Panel (Requirement M)
            item {
                Card(
                    colors = CardDefaults.cardColors(containerColor = BampCardBg),
                    shape = RoundedCornerShape(12.dp),
                    ...
                    Column(modifier = Modifier.padding(16.dp)) {
                        Row(verticalAlignment = Alignment.CenterVertically) {
                            Icon(Icons.Default.Science, contentDescription = null, tint = BampSecondary)
                            Spacer(modifier = Modifier.width(8.dp))
                            Text(
                                text = "Firestore Real-Time Test Panel (bamp-1de96)",
                                color = BampSecondary,
                                fontSize = 15.sp,
                                fontWeight = FontWeight.Bold
                            )
                        ...
                        Column(verticalArrangement = Arrangement.spacedBy(8.dp)) {
                            // Test Button 1: Create Test Patient
                            OutlinedButton(
                                onClick = {
                                    scope.launch {
                                        try {
                                            val testId = "TEST-${System.currentTimeMillis().toString().takeLast(4)}"
                                            ...
                                            firestoreRepository.addPatient(testPatient)
                                            Toast.makeText(context, "✅ Created Test Patient in bamp-1de96 Firestore!", Toast.LENGTH_LONG).show()
                                        } catch (e: Exception) {
                                ...
                                Icon(Icons.Default.Add, contentDescription = null, tint = BampSecondary)
                                Spacer(modifier = Modifier.width(8.dp))
                                Text("1. Create Test Patient", color = BampTextPrimary)
                            }

                            // Test Button 2: Upload Test Record
                            OutlinedButton(
                                onClick = {
                                    scope.launch {
                                        try {
                                            val dummyId = "PAT-TEST-${System.currentTimeMillis().toString().takeLast(4)}"
                                            firestoreRepository.saveXRayUploadMetadata(
                                                patientId = dummyId,
                                                imageUrl = "https://bamp-1de96.appspot.com/test_xray.dcm",
                                                filename = "lateral_ceph_test.dcm"
                                            )
                                            Toast.makeText(context, "✅ Uploaded Test X-Ray Metadata to xrayUploads collection!", Toast.LENGTH_LONG).show()
                                        } catch (e: Exception) {
                                            Toast.makeText(context, "Error: ${e.message}", Toast.LENGTH_SHORT).show()
                                        }
                                    }
                                ...
                                Icon(Icons.Default.CloudUpload, contentDescription = null, tint = BampSecondary)
                                Spacer(modifier = Modifier.width(8.dp))
                                Text("2. Upload Test Record", color = BampTextPrimary)
                            }

                            // Test Button 3: Read Test Record
                            OutlinedButton(
                                onClick = {
                                    Toast.makeText(context, "📖 Live Firestore Count: ${patients.size} Patients, ${predictions.size} Predictions synced from bamp-1de96!", Toast.LENGTH_LONG).show()
                                },
                                shape = RoundedCornerShape(8.dp),
                                modifier = Modifier.fillMaxWidth()
                            ) {
                                Icon(Icons.Default.Download, contentDescription = null, tint = BampSecondary)
                                Spacer(modifier = Modifier.width(8.dp))
                                Text("3. Read Test Record", color = BampTextPrimary)
                            }
                        }
                    }
                }
